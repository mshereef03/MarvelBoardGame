...
public abstract class Champion implements Damageable,Comparable {
	...
	public Champion deepClone() throws CloneNotSupportedException {
		Champion c = null;
		if(this instanceof Hero) {
			c= new Hero(name,maxHP,mana,maxActionPointsPerTurn,speed,attackRange,attackDamage);
		}
		else if(this instanceof Villain) {
			c= new Villain(name,maxHP,mana,maxActionPointsPerTurn,speed,attackRange,attackDamage);
		}
		else c= new AntiHero(name,maxHP,mana,maxActionPointsPerTurn,speed,attackRange,attackDamage);
		c.setCurrentHP(currentHP);
		c.setCurrentActionPoints(currentActionPoints);
		c.setCondition(condition);
		c.setLocation(new Point(location.x,location.y));
		ArrayList<Ability> ab = new ArrayList<Ability>();
		ArrayList<Effect> ef = new ArrayList<Effect>();
		
		for(Effect e : appliedEffects) {
			ef.add((Effect)e.clone());
		}
		c.setAbilities(ab);
		c.setAppliedEffects(ef);
