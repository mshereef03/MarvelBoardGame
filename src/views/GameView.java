...
//		name.setBorder(BorderFactory.createLineBorder(Color.BLACK,3));
		...
		//Not Working since we change the priority queue causing view and model turnOrders to not be in sync
...
//		}
//      This fixes this issue by using a deep clone of the turnOrder to maintain synchronization
		JPanel turns = new JPanel(new GridLayout(1,5));
		turns.setBounds(73,290,300,72);
		PriorityQueue temp = turnOrder.exactClone();
		temp.remove();
		while(!temp.isEmpty()) {
			Champion cur = (Champion)temp.remove();
			JTextArea tt = new JTextArea();
			tt.append(cur.getName());
			tt.setLineWrap(true);
			tt.setBorder(BorderFactory.createLineBorder(Color.BLACK,1));
